...
class HomeController extends DefaultChangeNotifier {
  ...
  Future<void> loadTotalTasks() async {
    ...
    todayTotalTasks = TotalTasksModel(
      ...
      totalTasksFinalizadas: todayTasks.where((task) => task.finished).length,
    );
    tomorrowTotalTasks = TotalTasksModel(
      totalTasks: tomorrowTasks.length,
      totalTasksFinalizadas: tomorrowTasks.where((task) => task.finished).length,
    );
    weekTotalTasks = TotalTasksModel(
      totalTasks: weekTasks.tasks.length,
      totalTasksFinalizadas: weekTasks.tasks.where((task) => task.finished).length,
    );
